'use strict'
$(document).on("deviceready", function () {//cordova plugins ready!
    //create or simply open database, when finalizing add checks for normal desktop browser to not use SQLLITE but webSQL directly
    _database = sqlitePlugin.openDatabase("GymBuddy.db", "1.0", "GymBuddy", 50000000,
        db => {
            console.log("Successfully opened GymBuddy database.", db);
            initiateDatabaseNew(db);
        },
        err => {
            console.log("Failed opening GymBuddy database.", err);
        }
    );

    function initiateDatabaseNew(db) {
        if (doesNotExist(localStorage.getItem("databaseSet"))) {
            const createQueries = [
                'CREATE TABLE serving (id NUMERIC, foodName TEXT, proteins REAL, carbs REAL, fats REAL, datetime INTEGER, date TEXT)',
                'CREATE TABLE macros (id NUMERIC, proteins REAL, carbs REAL, fats REAL, activeSince INTEGER)',
                'CREATE TABLE savedServing (id NUMERIC, foodName TEXT, proteins REAL, carbs REAL, fats REAL)'
            ];
            for (let query of createQueries)
                db.transaction(txn => {
                    console.log("transaction callback here", txn);
                    txn.executeSql(query,
                        [],
                        (tx, res) => {
                            console.log("query SUCCESS", tx, res);
                            alert("gotHere");
                        }
                    );
                });
